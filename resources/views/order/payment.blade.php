...
<div class="pay-card">
  ...
  <div class="pay-left">
    <div class="pay-bank-badge">
      <span class="pay-dot"></span> {{ $payment['bank_name'] }}
    </div>
    <h2>Quét mã QR</h2>
  ...
  <div class="pay-right">
    ...
    <div class="pay-info-grid">
      ...
      {{-- 🌟 ĐOẠN ĐƯỢC FIX: Kiểm tra theo trường cart_group_code thay vì "cart_" cũ để không bị nhận diện sai loại đơn --}}
      <div class="pay-info-item" style="grid-column:1/-1">
        <div class="pay-lbl">Nội dung mua</div>
        <div class="pay-val">
          @if($order->cart_group_code)
            @php
                $cartBooksCount = \App\Models\Order::where('cart_group_code', $order->cart_group_code)->count();
            @endphp
            Thanh toán giỏ hàng gộp ({{ $cartBooksCount }} cuốn sách)
          @elseif($order->book)
            Ebook: {{ $order->book->title }}
          @elseif($order->vipPlan)
            Gói hội viên VIP: {{ $order->vipPlan->name }}
          @else
            Đơn hàng dịch vụ Hiki
          @endif
        </div>
      </div>
    </div>

    {{-- 🌟 ĐOẠN ĐƯỢC FIX QUAN TRỌNG: Đọc chính xác giá trị đã cộng gộp từ $payment['amount'] thay vì $order->final_amount đơn lẻ --}}
    <div class="pay-amount-box">
      <div class="pay-lbl">Số tiền cần chuyển</div>
      <div class="pay-amount">{{ number_format($payment['amount']) }}<span>đ</span></div>
    </div>
    ...
    <div class="pay-note">
      Sau khi chuyển khoản thành công, nội dung sẽ được mở khóa hoặc kích hoạt tự động. Đảm bảo nội dung chuyển khoản chính xác tuyệt đối.
    </div>
  </div>
...
// Poll kiểm tra trạng thái thanh toán mỗi 3 giây
setInterval(function () {
  fetch("{{ route('book.check-payment', $order) }}")
    .then(r => r.json())
    .then(function (data) {
      if (data.status === 'paid') {
        ...
        box.innerHTML = `
          ...
          <div>
            ...
            <div style="font-size:12px;font-weight:400;margin-top:2px">
              Đang mở khóa dịch vụ...
            </div>
          </div>`;
        clearInterval(payTimer);
        
        // 🌟 ĐOẠN ĐƯỢC FIX: Điều hướng theo cart_group_code để đồng nhất dữ liệu
setTimeout(() => {
  @if($order->cart_group_code || ( $order->book_id && str_starts_with($order->book_id, 'cart_') ))
    window.location = "{{ route('account.purchased') }}"; // ĐƯA VỀ TRANG SÁCH ĐÃ MUA MỚI
  @elseif($order->book)
    window.location = "{{ route('home.render', $order->book->slug) }}"; 
  @else
    window.location = "/"; 
  @endif
}, 2000);
