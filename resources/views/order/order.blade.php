...
/* Kiểu hiển thị danh sách sách cho Giỏ hàng */
.cart-books-list {
    display: flex;
...
<div class="order-page">
    <div class="order-card">
        ...
        <div class="order-head">
            <div class="eyebrow">Xác nhận thanh toán</div>
            <div class="head-row">
        ...
        <div class="order-body">
            {{-- 🌟 ĐOẠN ĐƯỢC SỬA: Thay thế logic "cart_" cũ bằng việc lặp trực tiếp từ tập hợp $subOrders --}}
            @if($order->cart_group_code)
                <div class="info-row" style="align-items: flex-start;">
                    <span class="info-title">Sách đã chọn ({{ $subOrders->count() }})</span>
                    <span class="info-value">
                        <div class="cart-books-list">
                            @foreach($subOrders as $sub)
                                @if($sub->book)
                                    <div class="cart-book-item">• {{ $sub->book->title }}</div>
                                @endif
                            @endforeach
                        </div>
                    </span>
                ...
                <div class="info-row">
                    <span class="info-title">Quyền sở hữu</span>
                    <span class="info-value">Vĩnh viễn</span>
                </div>
            @elseif($order->book)
                <div class="info-row">
                    <span class="info-title">Ebook</span>
                    <span class="info-value">{{ $order->book->title }}</span>
                </div>
                <div class="info-row">
                    <span class="info-title">Tác giả</span>
            ...
            @elseif($order->vipPlan)
                <div class="info-row">
                    <span class="info-title">Gói dịch vụ</span>
                    <span class="info-value">Gói VIP {{ $order->vipPlan->name }}</span>
                </div>
                <div class="info-row">
                    <span class="info-title">Thời hạn</span>
                    <span class="info-value">{{ $order->vipPlan->months }} tháng</span>
                </div>
            @endif
            ...
            @if($order->coupon)
                <div class="info-row coupon-row">
                    ...
                    <span class="info-value">
                        <span style="margin-right: 8px;">{{ $order->coupon->code }}</span>
        ...
        {{-- 🌟 ĐOẠN ĐƯỢC SỬA: Đọc tổng tiền cộng dồn từ tập hợp $subOrders --}}
        @php
            $totalAmount = $subOrders->sum('amount');
            $totalDiscount = $subOrders->sum('discount');
            $totalFinalAmount = $subOrders->sum('final_amount');
        @endphp
        ...
        <div class="price-block">
            @if($totalDiscount > 0)
                <div class="price-original">
                    ...
                    <span class="value">{{ number_format($totalAmount) }}đ</span>
                </div>
            @endif
            <div class="price-final-row">
                <span class="label">Tổng thanh toán</span>
                <span class="value">{{ number_format($totalFinalAmount) }}đ</span>
            </div>
            @if($totalDiscount > 0)
                <div class="savings-tag">
                    Bạn tiết kiệm {{ number_format($totalDiscount) }}đ
                </div>
            @endif
        </div>
        ...
        <div class="actions">
            @if($order->cart_group_code)
                <a href="{{ route('cart.index') }}" class="btn-back">← Về giỏ hàng</a>
            @elseif($order->book)
                <a href="{{ route('home.watch', $order->book->slug) }}" class="btn-back">← Trở lại sách</a>
            @else
                <a href="/" class="btn-back">← Trang chủ</a>
            @endif

            <a href="{{ route('book.checkout', $order) }}" class="btn-pay">Tiếp tục thanh toán →</a>
        </div>
